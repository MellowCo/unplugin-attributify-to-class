<template>
	<tm-app ref="app" color="grey-5">
		<tm-form @submit="confirm" ref="form" v-model="show">
			<tm-form-item label="身份证号码" field="nameuser.a"
				:rules="[{required:true,message:'请输入66',validator:(val)=>val=='66'}]">
				<tm-input :inputPadding="[0,0]" v-model="show.nameuser.a" :transprent="true" :showBottomBotder="false">
				</tm-input>
			</tm-form-item>
			<tm-form-item label="选择日期" field="cale" :rules="[{required:true,message:'请选择日期哦'}]">
				<tm-input :inputPadding="[0,0]" :model-value.sync="show.cale[0]" @click="showCal=!showCal"
					suffix="tmicon-angle-right" placeholder="请选择有效日期" disabled :transprent="true"
					:showBottomBotder="false"></tm-input>
			</tm-form-item>
			<tm-form-item label="选择地区" field="city" :rules="[{required:true,message:'请选择地区'}]">
				<tm-input :inputPadding="[0,0]" :model-value.sync="show.cityStr" @click="showCity=!showCity"
			...
			<tm-form-item label="时间选择" field="time" :rules="[{required:true,message:'请选择时间'}]">
				<tm-input :inputPadding="[0,0]" v-model="show.timeStr" @click="showTimePickerView=!showTimePickerView"
					suffix="tmicon-angle-right" placeholder="请选择时间" disabled :transprent="true"
					:showBottomBotder="false"></tm-input>
			</tm-form-item>
			<tm-form-item label="弹出选择" field="time" :rules="[{required:true,message:'请选择水果种类'}]">
				<tm-input :inputPadding="[0,0]" :model-value.sync="show.pickerStr" @click="showPicker=!showPicker"
					suffix="tmicon-angle-right" placeholder="请选择你的水果种类" disabled :transprent="true"
					:showBottomBotder="false"></tm-input>
			</tm-form-item>
			...
			<tm-form-item label="价格选择" field="slider"
		...
		<tm-time-picker v-model="show.time" v-model:model-str="show.timeStr" v-model:show="showTimePickerView"
			:default-value="show.timeStr"></tm-time-picker>
		<tm-picker :columns="pickerlist" v-model="show.pickerIndex" v-model:model-str="show.pickerStr"
			v-model:show="showPicker" :default-value="show.pickerIndex"></tm-picker>
	</tm-app>
</template>
