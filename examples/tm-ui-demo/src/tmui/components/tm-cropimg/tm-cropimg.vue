<template>
	<view @longpress="" @touchstart="touchStart" @touchmove.stop.prevent="touchMove" @touchend="touchEnd"
		@touchcancel="touchEnd" @mousedown="touchStart" @mousemove.stop.prevent="touchMove" @mouseup="touchEnd"
		@mouseleave="touchEnd" class="relative overflow"
		:style="[{ width: `${view_width}px`, height: `${view_height}px`, background: '#000000' }]">
		<view v-if="!url" class="flex flex-center py-24"><text style="font-size: 24rpx;color: white;">请选择选择图片才可操作</text>
		</view>
		<!-- #ifndef APP-NVUE -->
		<canvas id="imgid" canvas-id="imgid" class=" l-0 t-0" :style="[
			{ 
				width: `${rectPos.w}px`, 
				height: `${rectPos.h}px`,
		...
		<image v-if="url" @error="imgError" class="absolute l-0 t-0" :class="[moveable?'opacity-10':'ani opacity-5']"
			:userInteractionEnabled="false" @load="imgLoad" :src="url" :style="[
		{
		  width: `${imgPos.w||1}px`,
		...
		<!-- 		   -->
		<view :class="[moveable?'':'ani']" :userInteractionEnabled="false" class="absolute l-0 t-0 rect" :style="[
      { width: rectPos.w + 'px', height: rectPos.h + 'px', transform: `translate(${rectPos.x}px,${rectPos.y}px)` }
    ]">

		</view>

		<view class="absolute l-0 b-0" :style="{ width: `${view_width}px`, height: `${80}px` }">
...
<script lang="ts" setup>
	 ...
	 */
	import {
		ref,getCurrentInstance,onMounted,nextTick
	} from "vue"
	...
	function getMoveTypeNumber(x: number, y: number) {
		if (x >= rectPos.value.x - 15 && x <= rectPos.value.x + 15 && y >= rectPos.value.y && y <= rectPos.value.y +
			rectPos.value.h
		...
		if (x >= rectPos.value.x + rectPos.value.w - 15 && x <= rectPos.value.x + rectPos.value.w + 15 && y >= rectPos
			.value.y && y <= rectPos.value.y + rectPos.value.h) {
			return 1;
		}
		if (x >= rectPos.value.x && x <= rectPos.value.x + rectPos.value.w && y >= rectPos.value.y + rectPos.value.h -
			15 && y <= rectPos.value.y + rectPos.value.h + 15) {
			return 1;
		}
		return 0
	}
