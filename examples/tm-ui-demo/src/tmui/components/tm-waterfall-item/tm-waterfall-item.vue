<template>
    <view ref="itemWall" class="absolute itemWall " :style="[
        ...
        <tm-sheet :margin="[0, 0]" :padding="[0, 0]" :round="props.round" :width="_width" unit="px" :color="props.color"
            ...
            <tm-image @click="onImgClick" :round="props.round" v-if="props.img" @load="imgLoadSuccess" :src="props.img" unit="px"
                :height="_nodeInfo.imgHeight" :width="_nodeInfo.imgWidth"></tm-image>
...
function nvuegetClientRect() {
    nextTick(function () {
        ...
        uni.createSelectorQuery().in(proxy).select('.itemWall').boundingClientRect(res => {
