 ...
 */
import { ComponentInternalInstance, computed, getCurrentInstance, inject, ref, watch, watchEffect } from 'vue'
import tmSheet from '../tm-sheet/tm-sheet.vue'
import tmIcon from '../tm-icon/tm-icon.vue'
import tmText from '../tm-text/tm-text.vue'
import tmTranslate from '../tm-translate/tm-translate.vue'
import type tmCheckboxGroup from '../tm-checkbox-group/tm-checkbox-group.vue'
...
function vailChecked() {
  ...
  const index = _groupCheckedVal.value.findIndex(el => el === props.value)
  if (index > -1)
    checked_val = true
...
<template>
  <view class="flex flex-row" :class="[_disabled ? 'opacity-5' : '']">
    ...
    <view class="flex flex-row flex-row-center-start flex-1" @click="hanlerClick">
      <tm-sheet
        v-if="props.custom"
        :event-penetration-enabled="true"
        :linear="props.linear"
      ...
      >
        <view v-if="!props.closeAni">
          <tm-translate v-if="_checked && !props.indeterminate" :duration="100" name="zoom" style="line-height: 1;">
            <tm-icon :font-size="props.size * 0.54" :name="props.icon" />
          </tm-translate>
          <tm-translate v-if="props.indeterminate" :duration="100" name="zoom" style="line-height: 1;">
            <tm-icon :font-size="props.size * 0.54" name="tmicon-minus" />
          </tm-translate>
        </view>
        <view v-if="props.closeAni">
          <tm-icon v-if="_checked && !props.indeterminate" :font-size="props.size * 0.54" :name="props.icon" />
          <tm-icon v-if="props.indeterminate" :font-size="props.size * 0.54" name="tmicon-minus" />
        </view>
      </tm-sheet>
      <slot>
        <tm-text :user-interaction-enabled="false" :font-size="props.fontSize" :label="props.label" />
      </slot>
    </view>
  </view>
</template>
