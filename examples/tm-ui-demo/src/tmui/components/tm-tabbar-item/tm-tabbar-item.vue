<template>
	<view class="flex flex-col flex-col-top-center" style="height: 75px;">
		<tm-sheet :height='60' :width='_width' :round='24' unit='px' _class='flex-center flex ' parenClass="relative"
			class="relative" :_style="_styletop" :followTheme="_btnTop && props.followTheme" :transprent="_transprent"
			:color="props.color" :margin="[0, 0]" :padding="_padding" :shadow="props.shadow" :outlined="props.outlined"
			...
			<tm-badge :fontSize="20" :color="c_font_style.dotColor" :eventPenetrationEnabled="true" :dot="props.dot"
				...
				<view :class="[_active ? 'anifun' : '']" class="flex flex-col flex-col-center-center "
					...
					<slot>
						<tm-icon v-if="!_load" _style="line-height: 0px;" :color="_color"
							:font-size="c_font_style.iconSize"
							:name="(_active ? c_font_style.icon : (c_font_style.unicon || c_font_style.icon))"></tm-icon>

					</slot>
					<tm-icon v-if="_load" spin _style="line-height: 0px;" :color="_color"
...
import { getCurrentInstance, computed, watchEffect, ref, provide, inject, onUpdated, onMounted, onUnmounted, nextTick, watch, PropType, ComponentInternalInstance } from 'vue';
...
const tmTabbarItemList = inject("tmTabbarItemList", computed<Array<string | number>>(() => []))
const nowUid = inject("tmTabbarUid", computed<string | number>(() => ""))
